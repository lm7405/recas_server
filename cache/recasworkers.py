...
class RecasWorker(multiprocessing.Process):
    def __init__(self, missionQ, resultQ):
    ...
    def run(self):
        ...
        while True:
            ...
            if next_mission is None:
                ...
                break
            # answer = next_mission()    # Mission object 호출(__call__() 실행)
            def test(count: int):
                sum = 0
                for k in range(count):
                    sum += k
                print("in mission, after for loop", sum)
                return sum
            answer = test(next_mission)
...
def Create_WorkerProcesses():
    ...
    workers = [RecasWorker(missionQ, resultQ)
